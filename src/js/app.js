...
// --- DYNAMIC MODULE LOADER FOR GITHUB PAGES ---
async function loadModularPartials() {
  console.log("Dynamically loading UI modules...");
  
  // Load Main Tabs
  const mainTabs = ['script', 'objections', 'tips', 'dbs', 'spanish', 'asep', 'sunday', 'dispo', 'lenders'];
  for (const t of mainTabs) {
    try {
      const res = await fetch(`tabs/${t}.html`);
      if (res.ok) {
        document.getElementById(`dynamic-load-page-${t}`).outerHTML = await res.text();
      }
    } catch (e) {
      console.warn("Could not load tab:", t, e);
    }
  }

  // Load Admin Main 
  try {
    const aRes = await fetch('admin/admin-main.html');
    if (aRes.ok) {
      document.getElementById('dynamic-load-page-admin').outerHTML = await aRes.text();
      
      // Once admin main is loaded, load its sub-tabs
      const adminTabs = ['overview', 'attendance', 'weekly', 'monthly', 'stats', 'profiles', 'coaching', 'monitoring', 'rebuttal', 'weekperf'];
      for (const at of adminTabs) {
        try {
          const atRes = await fetch(`admin/admin-${at}.html`);
          if (atRes.ok) {
            document.getElementById(`dynamic-load-admin-${at}`).outerHTML = await atRes.text();
          }
        } catch (e) {
          console.warn("Could not load admin sub-tab:", at, e);
        }
      }
    }
  } catch (e) {
    console.warn("Could not load admin shell", e);
  }
}

// Intercept window.onload or DOMContentLoaded to load partials first.
document.addEventListener('DOMContentLoaded', () => {
    loadModularPartials().then(() => {
        console.log("All modular UI components loaded seamlessly!");
        // Re-trigger any initialization logic if needed that assumes DOM is fully present
        if(typeof switchTab === 'function') {
           // Default to script tab if none active
           switchTab('script');
        }
    });
});
